...
- Baseline PWA/offline completata: Chart.js locale, prima release `releases/v2026.05.30/` scoped, release consigliata corrente `releases/v2026.06.12/`, launcher offline `/stable/`, finestra versioni e verifica Android del maintainer sulla baseline.
...
- Per verifica browser locale, provare prima il browser integrato se disponibile: creare un server HTTP temporaneo dal runtime browser/Node REPL, servire la root del repo (`nodeRepl.cwd`) e aprire `http://127.0.0.1:<porta-alta>/app/index.html`. Verificato il 2026-06-07 con porta `28575`: pagina caricata, titolo `Where's My Money?`, click su `Filtri`, navigazione Timeline/Statistiche/Impostazioni e nessun errore console. Chiudere sempre il server a fine prova.
- Per simulare l'esperienza Android durante i test locali, impostare nel browser integrato un viewport stretto prima della navigazione o prima del reload. Base consigliata: `393x852`; matrice utile per UI mobile: `360x800`, `393x852`, `412x915` e, se si toccano modali/filtri, una prova landscape. Questa simulazione aiuta a scoprire overflow, scroll, pannelli, bottom nav e layout, ma non sostituisce il test su Android reale per tastiera Gboard, autofill/suggerimenti, dettatura, gesture di sistema, installazione PWA e cache offline.
- Se il browser integrato o il Node REPL fallisce con errori di ambiente come `spawn setup refresh`, non usarlo come verdetto sulla UI: riprovare una volta piu tardi o usare il fallback Playwright headless.
- Fallback verificato il 2026-06-07: usare il Node bundled con `NODE_PATH` puntato ai moduli bundled, importare `playwright` e lanciare Chromium headless. Servire la root repo su `127.0.0.1:28575`, aprire `http://127.0.0.1:28575/app/index.html`, usare il selettore reale `#btn-filter-toggle` per aprire i filtri e controllare: nessun errore console/pageerror, nessuno script locale mancante, pannello filtri visibile.
- In questa workspace `npx` puo non essere visibile nel PATH della sessione Codex anche dopo l'installazione di Node; usare `C:\Program Files\nodejs\npx.cmd` oppure riavviare Codex per ricaricare il PATH. Se la cache browser di Playwright manca (`chromium_headless_shell-...` assente), installarla con `npx playwright install chromium`; in attesa della cache il pacchetto `playwright` bundled puo pilotare Edge/Chrome gia installati tramite `executablePath`.
