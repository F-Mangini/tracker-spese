...
La pressione prolungata sul pulsante filtri salva i filtri correnti come filtro rapido quando il pannello e completamente aperto; il filtro speciale `Selezionate` viene sempre escluso dallo snapshot. Con pannello chiuso o compatto, la stessa pressione sostituisce i filtri correnti con il filtro rapido e, se questo non e stato modificato, una nuova pressione ripristina i filtri precedenti. Se il filtro rapido viene modificato a mano, la pressione successiva lo riapplica; disattivandolo dopo questa riapplicazione vengono ricomposti il filtro rapido e i filtri presenti prima della prima attivazione. Un reset e trattato come una nuova base esplicita: dopo aver resettato e riattivato il filtro rapido, la disattivazione ripristina zero filtri. Lo snapshot rapido e persistito solo in `localStorage` sotto `${Storage.KEY}:quick-filter`, mentre lo snapshot di ritorno resta volatile per la sessione corrente.
