...
Per categorie, metodi e filtro `Selezionate`, un tap attiva o disattiva l'inclusione verde, mentre una pressione lunga attiva o disattiva l'esclusione rossa. Un tap su un elemento rosso lo rende verde; una pressione lunga su un elemento verde lo rende rosso. Se un gruppo non contiene inclusioni verdi, restano ammesse tutte le voci tranne quelle rosse; se contiene almeno un'inclusione, passano solo le voci verdi non escluse. Categorie e metodi si combinano tra loro, quindi per esempio si possono mostrare i soli pagamenti con carta escludendo contemporaneamente la categoria `bollette`. Gli stati esclusi fanno parte anche degli snapshot temporanei dei filtri usati dall'export custom, senza cambiare lo schema delle spese o il backup dati.
...
La pressione prolungata sul pulsante filtri salva i filtri correnti come filtro rapido quando il pannello e completamente aperto; il filtro speciale `Selezionate` viene sempre escluso dallo snapshot. Con pannello chiuso o compatto, se la configurazione corrente e diversa dal filtro rapido, la pressione lunga memorizza esattamente quella configurazione e applica il rapido. Quando invece i filtri coincidono gia con il rapido, la pressione lunga ripristina l'ultima configurazione diversa osservata prima di raggiungere quella corrispondenza. La regola vale sia se il rapido e stato applicato con pressione lunga sia se la stessa combinazione e stata raggiunta modificando i filtri a mano. Un reset e trattato come una nuova base esplicita: dopo aver resettato e riattivato il filtro rapido, la disattivazione ripristina zero filtri. Il gesto scatta dopo 450 ms e la durata viene verificata anche al rilascio vicino alla soglia, evitando una zona morta tra la soppressione del tap nativo e l'attivazione del filtro rapido. Lo snapshot rapido e persistito solo in `localStorage` sotto `${Storage.KEY}:quick-filter`, mentre lo snapshot di ritorno resta volatile per la sessione corrente.
...
- Le cache offline versionate esistono in cartelle release immutabili, oggi `releases/v2026.05.30/`, `releases/v2026.06.03/`, `releases/v2026.06.12/` e `releases/v2026.07.14/`; ogni nuova release richiede una nuova cartella e un nuovo cache namespace.
