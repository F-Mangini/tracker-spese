...
`/` e `/stable/` restano entrypoint comodi verso l'ultima stabile. Le PWA installabili/offline devono invece vivere dentro path versionati, per esempio `/releases/v2026.07.14/`, cosi una versione installata non cambia comportamento solo perche `main` viene aggiornato.
...
```json
{
  "recommended": "v2026.07.14",
  "releases": [
    {
      "id": "v2026.07.14",
      "path": "releases/v2026.07.14/",
      "date": "2026-07-14",
      "status": "recommended",
...
La prima release sorgente e `releases/v2026.05.30/`: e una baseline stabile con Chart.js locale, manifest scoped al proprio path e primo service worker release-scoped. La release consigliata corrente e `releases/v2026.07.14/`.
...
3. `releases/v2026.05.30/` e la prima release installabile con service worker scoped e cache offline; `releases/v2026.06.12/` resta disponibile come stabile storica e `releases/v2026.07.14/` e la release consigliata corrente.
...
```text
https://f-mangini.github.io/tracker-spese/releases/v2026.07.14/
```
...
3. Aprire la release consigliata `v2026.07.14`; dalla release verificare anche che `stable/latest` riporti alla stabile.
...
10. Chiudere e riaprire l'app installata: se dalla stable era stata scelta `v2026.07.14`, deve riaprire quella release.
...
```text
http://localhost:28575/releases/v2026.07.14/
```
